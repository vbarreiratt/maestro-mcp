...
// Multi-voice support interfaces
...
function expandNamedChord(chordName: string, octave: number = 4): string[] {
  ...
  const notesWithOctaves = normalizedChordNotes.map((note: string, index: number) => {
  ...
  if (bassNote) {
    ...
    const filteredUpperNotes = notesWithOctaves.filter((n: string) => {
 ...
 */
export function isMultiVoiceInput(input: any): input is MultiVoiceInput {
 ...
 */
export function parseMultiVoice(multiVoiceInput: MultiVoiceInput): VoiceResult[] {
  const context = {
    component: 'HybridParser',
  ...
  try {
    ...
    for (const voice of multiVoiceInput.voices) {
      const voiceDefaults: GlobalDefaults = {
        ...globalDefaults,
 ...
 */
export function detectNotationFormat(input: any): 'hybrid' | 'multi-voice' | 'legacy' {
 ...
 */
export function parseUnifiedNotation(input: any): VoiceResult[] {
  const format = detectNotationFormat(input);
  
  switch (format) {
    case 'multi-voice':
      return parseMultiVoice(input as MultiVoiceInput);
