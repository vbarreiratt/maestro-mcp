...
export class ProtocolHandler {
    ...
    static MIDI_NOTE_MIN = 0;
    static MIDI_NOTE_MAX = 127;
    static MIDI_VELOCITY_MIN = 0;
    static MIDI_VELOCITY_MAX = 127;
    static MIDI_CC_MIN = 0;
    static MIDI_CC_MAX = 127;
    static MIDI_CHANNEL_MIN = 1;
    static MIDI_CHANNEL_MAX = 16;
    static MIDI_PROGRAM_MIN = 0;
    static MIDI_PROGRAM_MAX = 127;
     ...
     */
    validateNoteParameters(note, velocity, channel) {
     ...
     */
    validateCCParameters(controller, value, channel) {
     ...
     */
    validateProgramChangeParameters(program, channel) {
    ...
    validateNote(note) {
        if (!Number.isInteger(note) || note < ProtocolHandler.MIDI_NOTE_MIN || note > ProtocolHandler.MIDI_NOTE_MAX) {
            throw new MidiError(`Invalid MIDI note: ${note}. Must be integer between ${ProtocolHandler.MIDI_NOTE_MIN} and ${ProtocolHandler.MIDI_NOTE_MAX}`, ErrorCodes.MIDI_INVALID_NOTE);
        }
    }
    /**
     * Validate normalized velocity (0.0-1.0)
     */
    validateNormalizedVelocity(velocity) {
        if (typeof velocity !== 'number' || velocity < ProtocolHandler.VELOCITY_NORMALIZED_MIN || velocity > ProtocolHandler.VELOCITY_NORMALIZED_MAX) {
            throw new MidiError(`Invalid velocity: ${velocity}. Must be number between ${ProtocolHandler.VELOCITY_NORMALIZED_MIN} and ${ProtocolHandler.VELOCITY_NORMALIZED_MAX}`, ErrorCodes.VALIDATION_FAILED);
        }
    }
    /**
     * Validate MIDI velocity (0-127)
     */
    validateMidiVelocity(velocity) {
        if (!Number.isInteger(velocity) || velocity < ProtocolHandler.MIDI_VELOCITY_MIN || velocity > ProtocolHandler.MIDI_VELOCITY_MAX) {
            throw new MidiError(`Invalid MIDI velocity: ${velocity}. Must be integer between ${ProtocolHandler.MIDI_VELOCITY_MIN} and ${ProtocolHandler.MIDI_VELOCITY_MAX}`, ErrorCodes.VALIDATION_FAILED);
        }
    }
    /**
    ...
    validateChannel(channel) {
        if (!Number.isInteger(channel) || channel < ProtocolHandler.MIDI_CHANNEL_MIN || channel > ProtocolHandler.MIDI_CHANNEL_MAX) {
            throw new MidiError(`Invalid MIDI channel: ${channel}. Must be integer between ${ProtocolHandler.MIDI_CHANNEL_MIN} and ${ProtocolHandler.MIDI_CHANNEL_MAX}`, ErrorCodes.VALIDATION_FAILED);
        }
    }
    /**
    ...
    validateController(controller) {
        if (!Number.isInteger(controller) || controller < ProtocolHandler.MIDI_CC_MIN || controller > ProtocolHandler.MIDI_CC_MAX) {
            throw new MidiError(`Invalid MIDI controller: ${controller}. Must be integer between ${ProtocolHandler.MIDI_CC_MIN} and ${ProtocolHandler.MIDI_CC_MAX}`, ErrorCodes.MIDI_INVALID_CONTROLLER);
        }
    }
    /**
     * Validate MIDI CC value (0-127)
     */
    validateCCValue(value) {
        if (!Number.isInteger(value) || value < ProtocolHandler.MIDI_CC_MIN || value > ProtocolHandler.MIDI_CC_MAX) {
            throw new MidiError(`Invalid MIDI CC value: ${value}. Must be integer between ${ProtocolHandler.MIDI_CC_MIN} and ${ProtocolHandler.MIDI_CC_MAX}`, ErrorCodes.VALIDATION_FAILED);
        }
    }
    /**
    ...
    validateProgram(program) {
        if (!Number.isInteger(program) || program < ProtocolHandler.MIDI_PROGRAM_MIN || program > ProtocolHandler.MIDI_PROGRAM_MAX) {
            throw new MidiError(`Invalid MIDI program: ${program}. Must be integer between ${ProtocolHandler.MIDI_PROGRAM_MIN} and ${ProtocolHandler.MIDI_PROGRAM_MAX}`, ErrorCodes.VALIDATION_FAILED);
        }
    }
    /**
     * Validate channel parameter (used for utility functions)
     */
    validateChannelParameter(channel) {
        this.validateChannel(channel);
    }
    /**
     * Convert normalized velocity (0-1) to MIDI velocity (0-127)
     */
    convertVelocity(normalizedVelocity) {
     ...
     */
    convertVelocityToNormalized(midiVelocity) {
    ...
    convertNoteNameToMidi(noteName) {
        ...
        if (!match) {
            throw new MidiError(`Invalid note format: ${noteName}. Use format like C4, F#3, Bb5`, ErrorCodes.MIDI_INVALID_NOTE);
        }
        const [, noteChar, accidental = '', octave] = match;
        if (!noteChar || octave === undefined) {
            throw new MidiError(`Invalid note format: ${noteName}`, ErrorCodes.MIDI_INVALID_NOTE);
        }
     ...
     */
    convertMidiToNoteName(midiNote) {
        this.validateNote(midiNote);
     ...
     */
    isContinuousController(controller) {
        this.validateController(controller);
     ...
     */
    createMidiMessage(status, data1, data2) {
     ...
     */
    createNoteOnMessage(channel, note, velocity) {
        this.validateChannel(channel);
        this.validateNote(note);
     ...
     */
    createNoteOffMessage(channel, note) {
        this.validateChannel(channel);
        this.validateNote(note);
     ...
     */
    createCCMessage(channel, controller, value) {
        this.validateChannel(channel);
        this.validateController(controller);
        this.validateCCValue(value);
