 ...
 */
export function expandChordAdvanced(chord, voicing = 'close', octave = 4) {
    try {
        ...
        switch (voicing) {
            ...
            case 'shell':
                ...
                if (notes.length >= 3) {
                    ...
                    if (notes.length >= 4)
                        shell.push(notes[3]); // add 7th if available
 ...
 */
export function generateScale(scaleName, octave = 4) {
    try {
 ...
 */
export function findEnharmonics(note) {
    try {
