...
import type { SearchCriteria, MaestroFormat } from '../schemas/library-schemas.js';

let libraryManager: HybridLibraryManager | null = null;

export function getLibraryManager(): HybridLibraryManager {
  if (!libraryManager) {
...
export async function searchLibrary(params: Record<string, unknown>) {
  ...
  try {
    ...
    const summary = results.slice(0, 3).map(r => 
      `${r.score.composer} - ${r.score.title} (${r.score.style}${r.score.composition_year ? `, ${r.score.composition_year}` : ''})`
    ).join('\n');
...
export async function playFromLibrary(params: Record<string, unknown>) {
  const validatedParams = PlayFromLibrarySchema.parse(params);
  const manager = getLibraryManager();
  
  try {
    let scoreId = validatedParams.score_id;
    let maestroFormat: MaestroFormat | null = null;
...
export async function getLibraryInfo(params: Record<string, unknown>) {
...
export async function addToLibrary(params: Record<string, unknown>) {
...
function applyModifications(format: MaestroFormat, modifications: Record<string, any>): MaestroFormat {
...
function generateScoreId(composer: string, title: string): string {
