...
export class SQLiteLibraryManager {
  ...
  async search(criteria: SearchCriteria): Promise<ScoreMetadata[]> {
  ...
  async getFullScore(scoreId: string): Promise<MaestroFormat | null> {
    ...
    return new Promise((resolve, reject) => {
      this.db.get(
        'SELECT maestro_format FROM scores WHERE id = ?',
        [scoreId],
        (err, row: any) => {
          if (err) {
  ...
  async addScore(score: FullScore): Promise<void> {
    ...
    const {
      ...
      instruments, tags, preview_notes, description
    } = score.metadata;
  ...
  async getStatistics(): Promise<{ total: number; composers: number; styles: number }> {
    await this.initialize();
