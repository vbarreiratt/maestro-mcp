...
import { PlanoMusical, PartituraExecutavel, MusicEvent } from '../../../schemas/music-schemas.js';
import { NoteEvent, CCEvent, SystemEvent } from '../../../schemas/midi-schemas.js';
import { validateMusicTheory } from './validators.js';
import { 
  expandChord, 
  parseMusicalTime, 
  normalizeVelocity, 
  calculateArticulation,
...
export class Tradutor {
  private defaultKey: string = 'C major';
  private defaultTimeSignature: string = '4/4';

  /**
   * Main transformation method: PlanoMusical → PartituraExecutável
   */
  async translateMusicalPlan(plano: PlanoMusical): Promise<PartituraExecutavel> {
    ...
    const controlChangeEvents: CCEvent[] = [];
    const systemEvents: SystemEvent[] = [];

    for (const event of plano.events) {
      const absoluteTime = parseMusicalTime(
        event.time, 
        plano.bpm, 
    ...
    // 6. Final validation
    const executableScore: PartituraExecutavel = {
      metadata,
      noteEvents,
      controlChangeEvents,
      systemEvents
   ...
   */
  private async processNoteEvent(event: MusicEvent, absoluteTime: number, bpm: number): Promise<NoteEvent[]> {
   ...
   */
  private async processChordEvent(event: MusicEvent, absoluteTime: number, bpm: number): Promise<NoteEvent[]> {
    if (typeof event.value !== 'string') {
   ...
   */
  private async processCCEvent(event: MusicEvent, absoluteTime: number): Promise<CCEvent> {
    if (typeof event.value !== 'object' || !event.value) {
      throw new Error(`CC event value must be object, got ${typeof event.value}`);
    }

    const ccData = event.value as { controller: number; value: number; description?: string };
  ...
  private async processSequenceEvent(
    ...
    baseTime: number, 
    bpm: number
  ): Promise<{ notes: NoteEvent[]; ccs: CCEvent[] }> {
    ...
    for (const seqEvent of event.value as MusicEvent[]) {
   ...
   */
  private secondsToMusicalTime(seconds: number, bpm: number): string {
...
export async function translateMusicalPlan(plano: PlanoMusical): Promise<PartituraExecutavel> {
