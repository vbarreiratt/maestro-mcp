...
export class MidiInterface {
    activePort = null; // JZZ MIDI port instance
    activePortName = null;
    messagesSent = 0;
    errorsCount = 0;
    ...
    ensureActivePort() {
        if (!this.activePort) {
            throw new MidiError('No active MIDI port. Connect to a port first using connectToPort()', ErrorCodes.MIDI_CONNECTION_FAILED);
        }
    }
    /**
    ...
    sendNoteOn(note, velocity, channel) {
        ...
        try {
            ...
            this.errorsCount++;
            throw new MidiError(`Failed to send MIDI Note On: ${error}`, ErrorCodes.MIDI_SEND_FAILED);
        }
    }
    /**
    ...
    sendNoteOff(note, channel) {
        ...
        try {
            ...
            this.errorsCount++;
            throw new MidiError(`Failed to send MIDI Note Off: ${error}`, ErrorCodes.MIDI_SEND_FAILED);
        }
    }
    /**
    ...
    sendCC(controller, value, channel) {
        ...
        try {
            ...
            this.errorsCount++;
            throw new MidiError(`Failed to send MIDI CC: ${error}`, ErrorCodes.MIDI_SEND_FAILED);
        }
    }
    /**
    ...
    sendProgramChange(program, channel) {
        ...
        try {
            ...
            this.errorsCount++;
            throw new MidiError(`Failed to send MIDI Program Change: ${error}`, ErrorCodes.MIDI_SEND_FAILED);
        }
    }
    /**
     * Send raw MIDI message (for advanced use cases)
     */
    sendRawMidiMessage(data) {
        this.ensureActivePort();
        try {
            ...
            this.errorsCount++;
            throw new MidiError(`Failed to send raw MIDI message: ${error}`, ErrorCodes.MIDI_SEND_FAILED);
        }
    }
    /**
    ...
    sendSysEx(data) {
        ...
        try {
            ...
            this.errorsCount++;
            throw new MidiError(`Failed to send SysEx message: ${error}`, ErrorCodes.MIDI_SEND_FAILED);
        }
    }
    /**
