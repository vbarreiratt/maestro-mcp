 ...
 */
export async function validateMusicTheory(plano: PlanoMusical): Promise<ValidationResult> {
  const errors: string[] = [];
  ...
  try {
    ...
    const eventValidations = await Promise.all(
      plano.events.map((event, index) => validateMusicEvent(event, index, plano))
    );
 ...
 */
async function validateMusicEvent(
  event: MusicEvent, 
  index: number, 
  plano: PlanoMusical
 ...
 */
async function validateNoteEvent(event: MusicEvent, index: number): Promise<ValidationResult> {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
async function validateChordEvent(
  event: MusicEvent, 
  index: number, 
  key?: string
): Promise<ValidationResult> {
 ...
 */
function validateCCEvent(event: MusicEvent, index: number): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  const ccData = event.value as Record<string, any>;
 ...
 */
async function validateSequenceEvent(event: MusicEvent, index: number): Promise<ValidationResult> {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  const sequenceEvents = event.value as MusicEvent[];
 ...
 */
function validateKeySignature(key: string): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
function validateHarmonicProgression(chordEvents: MusicEvent[], key?: string): ValidationResult {
  const warnings: string[] = [];
 ...
 */
function validateChordInKey(chord: string, key: string): ValidationResult {
  try {
 ...
 */
export async function validateMusicalConsistency(score: PartituraExecutavel): Promise<ValidationResult> {
  const errors: string[] = [];
  const warnings: string[] = [];
