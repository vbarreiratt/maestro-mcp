...
export function validateChordProgression(chords, _key) {
...
export function parseNoteDuration(notation, bpm) {
    ...
    if (type === 't')
        seconds *= 2 / 3; // Triplet
    if (type === 'd')
        seconds *= 3 / 2; // Dotted (different from musical dot)
    if (dotted === '.')
        seconds *= 1.5; // Musical dot
    return seconds;
}
export function expandChord(chord, _voicing = 'close', octave = 4) {
...
//# sourceMappingURL=music-schemas.js.map