...
export interface DAWInfo {
  ...
  confidence: number; // 0-1, how confident we are this DAW is running
}
...
export class PortManager {
  ...
  detectDAWs(): DAWInfo[] {
    ...
    const garageBandPorts = outputPorts.filter(p => 
      p.name.toLowerCase().includes('garageband') ||
      p.name.toLowerCase().includes('garage band')
    );
    
    if (garageBandPorts.length > 0) {
      // Prefer "Virtual In" ports for GarageBand
      const virtualInPort = garageBandPorts.find(p => 
        p.name.toLowerCase().includes('virtual in')
      ) || garageBandPorts[0];
    ...
    // Logic Pro Detection
    const logicPorts = outputPorts.filter(p => 
      p.name.toLowerCase().includes('logic') ||
    ...
    const abletonPorts = outputPorts.filter(p => 
      p.name.toLowerCase().includes('ableton') ||
    ...
    // Pro Tools Detection
    const proToolsPorts = outputPorts.filter(p => 
      p.name.toLowerCase().includes('pro tools') ||
    ...
    // FL Studio Detection
    const flStudioPorts = outputPorts.filter(p => 
      p.name.toLowerCase().includes('fl studio') ||
    ...
    const cubasePorts = outputPorts.filter(p => 
      p.name.toLowerCase().includes('cubase') ||
    ...
    const virtualPorts = outputPorts.filter(p => 
  ...
  suggestBestPort(targetDAW?: string): PortSuggestion {
    ...
    if (targetDAW && targetDAW.toLowerCase() !== 'auto') {
      ...
      const matchingDAW = detectedDAWs.find(daw => 
        daw.name.toLowerCase().includes(targetLower) ||
   ...
   */
  autoConnectOptimized(targetDAW?: string): { success: boolean; port?: string; reason: string; dawInfo?: DAWInfo } {
    try {
