 ...
 */
export class Mensageiro implements MensageiroMIDI {
  private midiInterface: MidiInterface;
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new MidiError(
        'MIDI system not initialized. Call initialize() first.',
        ErrorCodes.SYSTEM_INITIALIZATION_FAILED
      );
    }
  }
  ...
  async connectToPort(portName: string): Promise<boolean> {
    this.ensureInitialized();
