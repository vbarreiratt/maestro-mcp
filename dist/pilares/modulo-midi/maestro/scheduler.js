...
export class Scheduler {
    ...
    scheduleTonePartitura(partitura, playbackId) {
    ...
    scheduleNativePartitura(partitura, playbackId) {
    ...
    scheduleEvent(event, time) {
        ...
        if (this.useToneTransport) {
            ...
            switch (event.type) {
                case 'note':
                    this.scheduleNativeNote(event.data, playbackId);
                    break;
                case 'cc':
                    ...
                    if (delayMs === 0) {
                        // IMMEDIATE execution for time-0 events
                        setImmediate(() => {
                            this.triggerCCEvent(event.data, performance.now() / 1000);
                        });
                    }
                    else {
                ...
                case 'system':
                    ...
                    if (sysDelayMs === 0) {
                        // IMMEDIATE execution for time-0 events
                        setImmediate(() => {
                            this.triggerSystemEvent(event.data, performance.now() / 1000);
                        });
                    }
                    else {
    ...
    scheduleNote(noteEvent, playbackId) {
        // Schedule Note On
    ...
    scheduleNativeNote(noteEvent, playbackId) {
...
export class TimingCalculator {
     ...
     */
    static calculatePreciseDurations(rhythm, bpm, timeSignature = [4, 4]) {
        try {
     ...
     */
    static quantizeToMusicalGrid(positions, bpm, subdivision = 'sixteenth') {
        try {
     ...
     */
    static applySwing(positions, bpm, swingRatio = 0.67 // 67% swing (standard jazz)
    ) {
        try {
     ...
     */
    static calculateCumulativePositions(durations) {
        try {
            const positions = [0]; // Start at time 0
     ...
     */
    static validateMusicalTiming(positions, durations, bpm, timeSignature = [4, 4]) {
        const issues = [];
        const quarterNoteDuration = 60 / bpm;
     ...
     */
    static suggestOptimalTempo(noteDurations, targetDurationSeconds = 30) {
        try {
