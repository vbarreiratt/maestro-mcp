...
import { PlanoMusical } from '../../../schemas/music-schemas.js';
...
export class IntegrationTester {
   ...
   */
  private async testBasicNoteTranslation(): Promise<TestResult> {
    const startTime = Date.now();
    const testName = 'Basic Note Translation';

    try {
      const plano: PlanoMusical = {
        bpm: 120,
        timeSignature: '4/4',
        key: 'C major',
        events: [
   ...
   */
  private async testChordExpansion(): Promise<TestResult> {
    const startTime = Date.now();
   ...
   */
  private async testMusicalValidation(): Promise<TestResult> {
    const startTime = Date.now();
    const testName = 'Musical Validation';

    try {
      // Test valid plan
      const validPlano: PlanoMusical = {
        bpm: 120,
        timeSignature: '4/4',
        key: 'C major',
        events: [
      ...
      try {
        const invalidPlano: PlanoMusical = {
          bpm: 500, // Invalid BPM
          timeSignature: '4/4',
          events: [
      ...
      const passed = 
        validPartitura.noteEvents.length === 1 &&
        invalidPlanThrew;
   ...
   */
  private async testEndToEndMidiPlayback(): Promise<TestResult> {
    const startTime = Date.now();
   ...
   */
  private async testComplexProgression(): Promise<TestResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const passed = 
        partitura.noteEvents.length >= expectedMinNotes &&
