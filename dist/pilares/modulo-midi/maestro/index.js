...
export class Maestro {
    ...
    setupEventCallbacks() {
        ...
        this.scheduler.onNoteOn = (event, actualTime) => {
            this.eventManager.executeNoteEvent(event, actualTime);
        };
        this.scheduler.onNoteOff = (event, actualTime) => {
            const noteOffEvent = { ...event, velocity: 0 }; // Create a note-off version
            this.eventManager.executeNoteEvent(noteOffEvent, actualTime);
        };
        this.scheduler.onCCEvent = (event, actualTime) => {
            this.eventManager.executeCCEvent(event, actualTime);
        };
        this.scheduler.onSystemEvent = (event, actualTime) => {
            this.eventManager.executeSystemEvent(event, actualTime);
        };
        // Connect event manager callbacks to external listeners (Pilar 3)
        this.eventManager.onNoteEvent = (event) => this.onNoteEvent(event);
        this.eventManager.onCCEvent = (event) => this.onCCEvent(event);
        this.eventManager.onSystemEvent = (event) => this.onSystemEvent(event);
    }
    ensureInitialized() {
        if (!this.initialized) {
